...
<CodeGroup>
```bash cURL
curl -X PUT "http://127.0.0.1/updateCampaignData/0fff1da6-ace9-4811-9ed6-1d8aba00bc53" \
...
async function updateCampaignData(campaignId, redialConfig) {
  try {
    const response = await axios.put(
      `http://127.0.0.1/updateCampaignData/${campaignId}`,
      {
...
def update_campaign_data(campaign_id, redial_config):
    ...
    """
    url = f"http://127.0.0.1/updateCampaignData/{campaign_id}"
...
function updateCampaignData($campaignId, $redialConfig) {
    $url = "http://127.0.0.1/updateCampaignData/" . $campaignId;
