...
### cURL:
```bash
curl -X POST "https://carwale-backend.oriserve.com/updateKnowledgeBase" \
...
async function updateKnowledgeBase(filePath) {
  ...
  const response = await axios.post(
    'https://carwale-backend.oriserve.com/updateKnowledgeBase',
    form,
    {
      headers: {
        ...form.getHeaders(),
...
def update_knowledge_base(file_path):
    url = "https://carwale-backend.oriserve.com/updateKnowledgeBase"
