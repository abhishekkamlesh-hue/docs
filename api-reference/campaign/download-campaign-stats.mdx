...
```bash cURL
curl --request GET \
  --url 'https://carwale-backend.oriserve.com/downloadCampaignStats?campaign_id={campaign_id}&format=xlsx' \
...
fetch(`https://carwale-backend.oriserve.com/downloadCampaignStats?campaign_id=${campaignId}&format=${format}`, {
  credentials: 'include'
})
