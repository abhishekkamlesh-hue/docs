...
```bash cURL
curl --request GET \
  --url 'https://carwale-backend.oriserve.com/campaign/{id}'
```
...
fetch(`https://carwale-backend.oriserve.com/campaign/${campaignId}`, options)
...
url = f"https://carwale-backend.oriserve.com/campaign/{campaign_id}"
