...
### cURL:
```bash
curl -X POST "https://carwale-backend.oriserve.com/getAgentConfigAndUserDetailsForInboundCall" \
...
async function getInboundCallConfig(phoneNumber, callerNumber) {
  try {
    const response = await axios.post(
      'https://carwale-backend.oriserve.com/getAgentConfigAndUserDetailsForInboundCall',
      {
