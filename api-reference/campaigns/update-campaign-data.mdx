...
<CodeGroup>
```bash cURL
curl -X PUT "https://unabject-nonhereditably-gayla.ngrok-free.dev/updateCampaignData/0fff1da6-ace9-4811-9ed6-1d8aba00bc53" \
...
async function updateCampaignData(campaignId, redialConfig) {
  try {
    const response = await axios.put(
      `https://unabject-nonhereditably-gayla.ngrok-free.dev/updateCampaignData/${campaignId}`,
      {
    ...
    console.log('✅ Campaign updated:', response.data.data.campaignId);
...
def update_campaign_data(campaign_id, redial_config):
    ...
    """
    url = f"https://unabject-nonhereditably-gayla.ngrok-free.dev/updateCampaignData/{campaign_id}"
    ...
    try:
        ...
        print(f"✅ Campaign updated: {result['data']['campaignId']}")
...
function updateCampaignData($campaignId, $redialConfig) {
    $url = "https://unabject-nonhereditably-gayla.ngrok-free.dev/updateCampaignData/" . $campaignId;
    ...
    if ($httpCode === 200) {
        ...
        echo "✅ Campaign updated: " . $result['data']['campaignId'] . "\n";
...
<AccordionGroup>
  <Accordion title="📊 Campaign Information" icon="chart-line">
  ...
  <Accordion title="⏰ Scheduling Configuration" icon="clock">
  ...
  <Accordion title="🔄 Redial Configuration" icon="redo">
...
<AccordionGroup>
  <Accordion title="🎯 Redial Strategy" icon="target">
  ...
  <Accordion title="🔧 Configuration Guidelines" icon="wrench">
  ...
  <Accordion title="🛡️ Error Prevention" icon="shield">
...
<CardGroup cols={2}>
  <Card 
    ...
    icon="eye" 
    href="/api-reference/campaign/get-campaign"
  >
    Retrieve detailed information about a specific campaign including current redial configuration
  </Card>
  
  <Card 
    title="Get All Campaigns" 
    icon="list" 
    href="/api-reference/campaign/get-all-campaigns"
  >
    List all campaigns with their current status and configuration summary
  </Card>
  
  <Card 
    title="Start Campaign" 
    icon="play" 
    href="/api-reference/campaign/start-campaign"
  >
    Begin campaign execution with the configured redial settings
  </Card>
  
  <Card 
    title="Pause Campaign" 
    icon="pause" 
    href="/api-reference/campaign/pause-campaign"
  >
    Temporarily pause campaign execution while preserving redial configuration
  </Card>
</CardGroup>