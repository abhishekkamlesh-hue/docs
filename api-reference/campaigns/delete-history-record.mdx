...
const response = await fetch(`https://carwale-backend.oriserve.com/deleteHistoryRecord/${callId}`, {
  method: 'DELETE'
});
...
response = requests.delete(f'https://carwale-backend.oriserve.com/deleteHistoryRecord/{call_id}')
...
```bash cURL
curl -X DELETE https://carwale-backend.oriserve.com/deleteHistoryRecord/call-uuid-abc123
```
