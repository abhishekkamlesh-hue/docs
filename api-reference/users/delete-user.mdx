...
### cURL:
```bash
curl -X DELETE "https://carwale-backend.oriserve.com/deleteUser?userId=507f1f77bcf86cd799439011" \
...
async function deleteUser(userId) {
  try {
    const response = await axios.delete(
      `https://carwale-backend.oriserve.com/deleteUser?userId=${userId}`,
      {
        headers: {
...
def delete_user(user_id):
    url = f"https://carwale-backend.oriserve.com/deleteUser?userId={user_id}"
