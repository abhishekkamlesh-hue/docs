...
### cURL:
```bash
curl -X POST https://carwale-backend.oriserve.com/createUser \
...
async function createUser(userData) {
  try {
    const response = await axios.post(
      'https://carwale-backend.oriserve.com/createUser',
      {
        email: 'john.doe@company.com',
        password: 'SecureP@ss123',
        first_name: 'John',
...
def create_user():
    url = "https://carwale-backend.oriserve.com/createUser"
