...
### cURL:
```bash
curl -X POST "https://carwale-backend.oriserve.com/updateUser" \
...
async function updateUser(userId, updates) {
  try {
    const response = await axios.post(
      'https://carwale-backend.oriserve.com/updateUser',
      {
        userId,
        ...updates
      },
...
def update_user(user_id, **updates):
    url = "https://carwale-backend.oriserve.com/updateUser"
